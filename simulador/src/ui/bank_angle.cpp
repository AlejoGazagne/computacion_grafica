...
namespace UI {
    ...
    bool BankAngleIndicator::initializeOpenGL() {
        ...
        if (!shader_manager.loadShader(shader_name_, 
                                      "shaders/vertex_bank_angle.glsl", 
                                      "shaders/fragment_bank_angle.glsl")) {
            std::cerr << "Failed to load HUD shaders" << std::endl;
            return false;
        }
    ...
    void BankAngleIndicator::updateScreenSize(int width, int height) {
        screen_width_ = width;
    ...
    void BankAngleIndicator::render(float bank_angle) {
        ...
        float normalized_angle = bank_angle;
        ...
        for (int i = center_line_index - 2; i <= center_line_index + 2; i++) {
            ...
            if (t >= 0.0f && t <= 1.0f) {
                ...
                if (i % 2 == 0 && line_angle != 0.0f) {  // Cada 20°, excluyendo 0°
                ...
                visible_lines++;
                if (visible_lines >= 5) break;  // Limitar a máximo 5 líneas
            }
        }
