...
namespace Scene
{
    ...
    Camera::Camera()
        : config_(), position_(config_.position), yaw_(DEFAULT_YAW), pitch_(DEFAULT_PITCH), roll_(0.0f),
          ...
          cinematic_desired_position_(0.0f), cinematic_needs_reposition_(true),
          cinematic_lateral_side_(1)
    {
    ...
    void Camera::setCinematicFollow(const glm::vec3 &target_position,
                                    ...
                                    float delta_time)
    {
        ...
        float current_distance = glm::length(position_ - target_position);

        // Si necesita reposicionarse o es la primera vez, calcular nueva posición FIJA
        if (cinematic_needs_reposition_ || current_distance > cinematic_max_distance_relocate_)
        {
            // Calcular punto por delante del avión usando su vector forward
            glm::vec3 point_ahead = target_position + target_forward * cinematic_forward_distance_;

            // Aplicar offset lateral usando el vector right del avión
            // Alternar entre derecha e izquierda usando cinematic_lateral_side_
            glm::vec3 lateral_offset = target_right * (cinematic_lateral_offset_ * cinematic_lateral_side_);

            // Aplicar offset de altura usando el vector up del avión
            glm::vec3 height_offset = target_up * cinematic_height_offset_;

            // Posición FIJA de la cámara - se establece y NO SE MUEVE hasta el próximo reposicionamiento
            position_ = point_ahead + lateral_offset + height_offset;

            // Alternar el lado para el próximo reposicionamiento
            cinematic_lateral_side_ *= -1;

            // Marcar que ya no necesita reposicionarse
            cinematic_needs_reposition_ = false;
        }

        // La cámara NO SE MUEVE, solo permanece fija en su posición
        // ÚNICAMENTE rotamos la cámara para que siempre mire hacia el avión
        lookAt(target_position);

        matrices_dirty_ = true;
    }

    void Camera::setCinematicParameters(float forward_distance, float lateral_offset,
                                        float height_offset, float max_distance_relocate,
                                        float smooth_factor)
    {
        // Solo forzar reposicionamiento si cambian parámetros que afectan la posición actual
        bool position_params_changed = (forward_distance != cinematic_forward_distance_ ||
                                        lateral_offset != cinematic_lateral_offset_ ||
                                        height_offset != cinematic_height_offset_);

        cinematic_forward_distance_ = forward_distance;
        cinematic_lateral_offset_ = lateral_offset;
        cinematic_height_offset_ = height_offset;
        cinematic_max_distance_relocate_ = max_distance_relocate;
        cinematic_smooth_factor_ = glm::clamp(smooth_factor, 0.0f, 1.0f);

        // Solo forzar reposicionamiento si cambiaron parámetros de posición
        if (position_params_changed)
        {
            cinematic_needs_reposition_ = true;
        }
    }
