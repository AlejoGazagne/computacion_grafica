...
namespace Graphics {
    namespace Rendering {
        ...
        Mesh::Mesh() : name_("unnamed_mesh"), initialized_(false) {
        }

        Mesh::Mesh(const std::string& name) : name_(name), initialized_(false) {
        }

        Mesh::Mesh(const std::vector<Vertex>& vertices, const std::vector<unsigned int>& indices, const std::string& name)
            : vertices_(vertices), indices_(indices), name_(name.empty() ? "unnamed_mesh" : name), initialized_(false) {
            setupMesh();
        }
